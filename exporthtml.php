...
/*
//TDMU - export classbook as HTML document to open it in the MS Word
*/
...
$url = new moodle_url('/mod/checklist/exporttml.php', array('id' => $cm->id));
$PAGE->set_url($url);
require_login($course, true, $cm);

if ($CFG->version < 2011120100) {//@TDMU-01
...
$items = $DB->get_records_select('checklist_item', "checklist = ? AND userid = 0", array($checklist->id), 'position');//@TDMU-01
if (!$items) {
...
if ($users && !empty($users)) {//@TDMU-01 2.0-conditions only
    $users = array_slice($users, $page*$perpage, $perpage);
    list($usql, $uparams) = $DB->get_in_or_equal($users);//@TDMU-01
//var_dump($users); //debug
    ...
    $ausers = $DB->get_records_sql('SELECT u.id, u.firstname, u.lastname FROM {user} u WHERE u.id '.$usql.' ORDER BY '.$orderby, $uparams);//@TDMU-01
    }//@TDMU-01 2.0
}
...
if ($ausers) {
	...
	foreach ($ausers as $auser) {
        ...
        $sql .= 'ON (i.id = c.item AND c.userid = ? ) WHERE i.checklist = ? AND i.userid=0 ORDER BY i.position';//@TDMU-01
        $checks = $DB->get_records_sql($sql, array($auser->id, $checklist->id) );//@TDMU-01
